...
-- (e11 `op1` e12) `op2` e2
mkOpAppRn e1@(L _ (OpApp x e11 op1 fix1 e12)) op2 fix2 e2
  | nofix_error
  = do precParseErr (get_op op1,fix1) (get_op op2,fix2)
       return (OpApp x e1 op2 fix2 e2)

  | associate_right = do
    new_e <- mkOpAppRn e12 op2 fix2 e2
    return (OpApp x e11 op1 fix1 (L loc' new_e))
  where
...
mkOpAppRn e1@(L _ (NegApp _ neg_arg neg_name)) op2 fix2 e2
  ...
  = do precParseErr (NegateOp,negateFixity) (get_op op2,fix2)
       return (OpApp noExt e1 op2 fix2 e2)
...
mkOpAppRn e1 op1 fix1 e2@(L _ (NegApp {}))     -- NegApp can occur on the right
  ...
  = do precParseErr (get_op op1, fix1) (NegateOp, negateFixity)
       return (OpApp noExt e1 op1 fix1 e2)
  where
...
mkOpAppRn e1 op fix e2                  -- Default case, no rearrangment
  = ASSERT2( right_op_ok fix (unLoc e2),
    ...
    return (OpApp noExt e1 op fix e2)
...
right_op_ok fix1 (OpApp _ _ _ fix2 _)
...
checkSectionPrec direction section op arg
  = case unLoc arg of
        OpApp _ _ op' fix _ -> go_for_it (get_op op') fix
