...
maybemodwarning :: { ([AddAnn],Maybe WarningTxt) }
...
maybeimpspec :: { ([AddAnn],Located (Maybe (Bool, [LIE RdrName]))) }
        : impspec                  { (fst $1
                                     ,L (gl (snd $1)) (Just (unLoc (snd $1)))) }
        | {- empty -}              { ([],noLoc Nothing) }

impspec :: { ([AddAnn],Located (Bool, [LIE RdrName])) }
        :  '(' exportlist ')'                 { ([mo $1,mc $3]
                                                ,sLL $1 $> (False, fromOL $2)) }
...
overlap_pragma :: { ([AddAnn],Maybe OverlapMode) }
...
strict_mark :: { Located HsBang }
        ...
        | '{-# UNPACK' '#-}'         {% ams (sLL $1 $> (HsUserBang (Just True)  False))
                                            [mo $1,mc $2] }
        | '{-# NOUNPACK' '#-}'       {% ams (sLL $1 $> (HsUserBang (Just False) True))
                                            [mo $1,mc $2] }
        | '{-# UNPACK' '#-}' '!'     {% ams (sLL $1 $> (HsUserBang (Just True)  True))
                                            [mo $1,mc $2] }
        | '{-# NOUNPACK' '#-}' '!'   {% ams (sLL $1 $> (HsUserBang (Just False) True))
                                               [mo $1,mc $2] }
...
gadt_constr :: { Located ([AddAnn],LConDecl RdrName) }
...
deriving :: { Located ([AddAnn]
                      ,Maybe [LHsType RdrName]) }
        : {- empty -}                     { noLoc ([],Nothing) }
        | 'deriving' qtycon               { let { L loc tv = $2 }
                                            in sLL $1 $> ([mj AnnDeriving $1]
                                                  ,Just [L loc (HsTyVar tv)]) }
        | 'deriving' '(' ')'              { sLL $1 $> ([mj AnnDeriving $1,mo $2,mc $3]
                                               ,Just []) }
        | 'deriving' '(' inst_types1 ')'  { sLL $1 $> ([mj AnnDeriving $1,mo $2,mc $4]
                                               ,Just $3) }
...
sigdecl :: { Located (OrdList (LHsDecl RdrName)) }
        ...
        | '{-# SPECIALISE_INLINE' activation qvar '::' sigtypes1 '#-}'
             { sLL $1 $> $ toOL [ sLL $1 $> $ SigD (SpecSig $3 t
...
--                                        (getSTRING $1) placeHolderType) }
        ...
        | '[' list ']'                  {% ams (sLL $1 $> (unLoc $2)) [mo $1,mc $3] }
        | '[:' parr ':]'                {% ams (sLL $1 $> (unLoc $2)) [mo $1,mc $3] }
        | '_'                           { sL1 $1 EWildPat }
...
list :: { LHsExpr RdrName }
        ...
        | lexps   { sL1 $1 $ ExplicitList placeHolderType Nothing
                                                   (reverse (unLoc $1)) }
        | texp '..'             {% ams (sLL $1 $> $ ArithSeq noPostTcExpr Nothing
                                       (From $1)) [mj AnnDotdot $2] }
        | texp ',' exp '..'     {% ams (sLL $1 $> $ ArithSeq noPostTcExpr Nothing
                                                               (FromThen $1 $3))
                                       [mj AnnComma $2,mj AnnDotdot $4] }
        | texp '..' exp         {% ams (sLL $1 $> $ ArithSeq noPostTcExpr Nothing
                                       (FromTo $1 $3)) [mj AnnDotdot $2] }
        | texp ',' exp '..' exp {% ams (sLL $1 $> $ ArithSeq noPostTcExpr Nothing
                                       (FromThenTo $1 $3 $5))
                                       [mj AnnComma $2,mj AnnDotdot $4] }
...
parr :: { LHsExpr RdrName }
        ...
        | lexps                { sL1 $1 $ ExplicitPArr placeHolderType
                                                          (reverse (unLoc $1)) }
        | texp '..' exp        {% ams (sLL $1 $> $ PArrSeq noPostTcExpr (FromTo $1 $3))
                                      [mj AnnDotdot $2] }
        | texp ',' exp '..' exp
                        {% ams (sLL $1 $> $ PArrSeq noPostTcExpr (FromThenTo $1 $3 $5))
                               [mj AnnComma $2,mj AnnDotdot $4] }
        | texp '|' flattenedpquals
                        {% ams (sLL $1 $> $ mkHsComp PArrComp (unLoc $3) $1)
                               [mj AnnVbar $2] }
...
alt_rhs :: { Located (GRHSs RdrName (LHsExpr RdrName)) }
        : ralt wherebinds             { sLL $1 $> (GRHSs (unLoc $1) (snd $ unLoc $2)) }

ralt :: { Located [LGRHS RdrName (LHsExpr RdrName)] }
        : '->' exp             {% ams (sLL $1 $> (unguardedRHS $2)) [mj AnnRarrow $1] }
