...
exp   :: { LHsExpr GhcPs }
        : infixexp '::' sigtype {% ams (sLL $1 $> $ ExprWithTySig (mkLHsSigWcType $3) $1)
                                       [mu AnnDcolon $2] }
        | infixexp '-<' exp     {% ams (sLL $1 $> $ HsArrApp noExt $1 $3 placeHolderType
                                                        HsFirstOrderApp True)
                                       [mu Annlarrowtail $2] }
        | infixexp '>-' exp     {% ams (sLL $1 $> $ HsArrApp noExt $3 $1 placeHolderType
...
infixexp :: { LHsExpr GhcPs }
        : exp10 { $1 }
        | infixexp qop exp10  {% ams (sLL $1 $> (OpApp noExt $1 $2 placeHolderFixity $3))
                                     [mj AnnVal $2] }
...
infixexp_top :: { LHsExpr GhcPs }
        ...
        | infixexp_top qop exp10_top
                                  {% ams (sLL $1 $> (OpApp noExt $1 $2 placeHolderFixity $3))
                                         [mj AnnVal $2] }
...
fexp    :: { LHsExpr GhcPs }
        ...
        | fexp TYPEAPP atype         {% ams (sLL $1 $> $ HsAppType (mkHsWildCardBndrs $3) $1)
                                            [mj AnnAt $2] }
        | 'static' aexp              {% ams (sLL $1 $> $ HsStatic noExt placeHolderNames $2)
                                            [mj AnnStatic $1] }
        | aexp                       { $1 }
...
list :: { ([AddAnn],HsExpr GhcPs) }
        : texp    { ([],ExplicitList noExt placeHolderType Nothing [$1]) }
        | lexps   { ([],ExplicitList noExt placeHolderType Nothing
                                                   (reverse (unLoc $1))) }
        | texp '..'             { ([mj AnnDotdot $2],
                                      ArithSeq noExt noPostTcExpr Nothing (From $1)) }
        | texp ',' exp '..'     { ([mj AnnComma $2,mj AnnDotdot $4],
                                  ArithSeq noExt noPostTcExpr Nothing
                                                             (FromThen $1 $3)) }
...
parr :: { ([AddAnn],HsExpr GhcPs) }
        :                      { ([],ExplicitPArr noExt placeHolderType []) }
        | texp                 { ([],ExplicitPArr noExt placeHolderType [$1]) }
        | lexps                { ([],ExplicitPArr noExt placeHolderType
                                                          (reverse (unLoc $1))) }
        | texp '..' exp        { ([mj AnnDotdot $2]
                                 ,PArrSeq noExt noPostTcExpr (FromTo $1 $3)) }
        | texp ',' exp '..' exp
                        { ([mj AnnComma $2,mj AnnDotdot $4]
                          ,PArrSeq noExt noPostTcExpr (FromThenTo $1 $3 $5)) }
        | texp '|' flattenedpquals
