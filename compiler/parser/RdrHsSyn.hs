...
mkGadtDecl _ ty@(L _ (HsForAllTy _ (Just l) _ _ _))
  = parseErrorSDoc l $
    ...
    ppr ty
mkGadtDecl names (L ls (HsForAllTy imp Nothing qvars cxt (L _ (HsForAllTy _ _ qvars1 cxt1 tau))))
  = mkGadtDecl names (L ls (HsForAllTy imp Nothing (qvars `mappend` qvars1) (L (getLoc cxt) (unLoc cxt ++ unLoc cxt1)) tau))
