...
checkAPat msg loc e0 = do
 ...
 case e0 of
   ...
   ExplicitList _ _ _ es  -> do ps <- mapM (checkLPat msg) es
                                return (ListPat noExt ps placeHolderType Nothing)
   ExplicitPArr _ _ es  -> do ps <- mapM (checkLPat msg) es
   ...
   ExplicitSum _ alt arity expr _ -> do
...
splitBang (L _ (OpApp _ l_arg bang@(L _ (HsVar _ (L _ op))) _ r_arg))
...
isFunLhs e = go e [] []
 where
   ...
   go e@(L loc (OpApp _ l (L loc' (HsVar _ (L _ op))) fix r)) es ann
                 ...
                 Just (op', Infix, j : k : es', ann')
                   ...
                   where
                     op_app = L loc (OpApp noExt k
                                       (L loc' (HsVar noExt (L loc' op))) fix r)
                 _ -> return Nothing }
...
checkCmd _ (HsArrApp _ e1 e2 ptt haat b) =
...
checkCmd _ (HsDo _ DoExpr (L l stmts) ty) =
    mapM checkCmdLStmt stmts >>= (\ss -> return $ HsCmdDo (L l ss) ty)

checkCmd _ (OpApp _ eLeft op _fixity eRight) = do
...
mkSumOrTuple Unboxed _ (Sum alt arity e) =
    return (ExplicitSum noExt alt arity e PlaceHolder)
