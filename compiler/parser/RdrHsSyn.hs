...
mkTyClD :: LTyClDecl x n -> LHsDecl x n
mkTyClD (L loc d) = L loc (TyClD d)

mkInstD :: LInstDecl x n -> LHsDecl x n
mkInstD (L loc d) = L loc (InstD d)

mkClassDecl :: SourceTextX x => SrcSpan
            -> Located (Maybe (LHsContext x RdrName), LHsType x RdrName)
            -> Located (a,[Located (FunDep (Located RdrName))])
            -> OrdList (LHsDecl x RdrName)
            -> P (LTyClDecl x RdrName)
...
mkATDefault :: SourceTextX x => LTyFamInstDecl x RdrName
            -> Either (SrcSpan, SDoc) (LTyFamDefltEqn x RdrName)
...
mkTyData :: SourceTextX x
         => SrcSpan
         -> NewOrData
         -> Maybe (Located CType)
         -> Located (Maybe (LHsContext x RdrName), LHsType x RdrName)
         -> Maybe (LHsKind x RdrName)
         -> [LConDecl x RdrName]
         -> HsDeriving x RdrName
         -> P (LTyClDecl x RdrName)
...
mkDataDefn :: SourceTextX x
           => NewOrData
           -> Maybe (Located CType)
           -> Maybe (LHsContext x RdrName)
           -> Maybe (LHsKind x RdrName)
           -> [LConDecl x RdrName]
           -> HsDeriving x RdrName
           -> P (HsDataDefn x RdrName)
...
mkTySynonym :: SourceTextX x
            => SrcSpan
            -> LHsType x RdrName  -- LHS
            -> LHsType x RdrName  -- RHS
            -> P (LTyClDecl x RdrName)
...
mkTyFamInstEqn :: SourceTextX x
               => LHsType x RdrName
               -> LHsType x RdrName
               -> P (TyFamInstEqn x RdrName,[AddAnn])
...
mkTyFamInst :: SrcSpan
            -> LTyFamInstEqn x RdrName
            -> P (LInstDecl x RdrName)
mkTyFamInst loc eqn
  = return (L loc (TyFamInstD (TyFamInstDecl { tfid_eqn  = eqn
                                             , tfid_fvs  = placeHolderNames })))

mkFamDecl :: SourceTextX x
          => SrcSpan
          -> FamilyInfo x RdrName
          -> LHsType x RdrName                   -- LHS
          -> Located (FamilyResultSig x RdrName) -- Optional result signature
...
mkSpliceDecl :: LHsExpr x RdrName -> HsDecl x RdrName
-- If the user wrote
...
cvTopDecls :: OrdList (LHsDecl x RdrName) -> [LHsDecl x RdrName]
cvTopDecls decls = go (fromOL decls)
  where
    go :: [LHsDecl x RdrName] -> [LHsDecl x RdrName]
    go []                   = []
...
cvBindGroup :: SourceTextX x
            => OrdList (LHsDecl x RdrName) -> P (HsValBinds x RdrName)
cvBindGroup binding
  = do { (mbs, sigs, fam_ds, tfam_insts, dfam_insts, _) <- cvBindsAndSigs binding
       ; ASSERT( null fam_ds && null tfam_insts && null dfam_insts)
         return $ ValBindsIn mbs sigs }

cvBindsAndSigs :: SourceTextX x => OrdList (LHsDecl x RdrName)
  -> P (LHsBinds x RdrName, [LSig x RdrName], [LFamilyDecl x RdrName]
         , [LTyFamInstDecl x RdrName], [LDataFamInstDecl x RdrName], [LDocDecl])
...
getMonoBind :: LHsBind x RdrName -> [LHsDecl x RdrName]
  -> (LHsBind x RdrName, [LHsDecl x RdrName])
...
has_args :: [LMatch x RdrName (LHsExpr x RdrName)] -> Bool
...
splitCon :: SourceTextX x => LHsType x RdrName
      -> P (Located RdrName, HsConDeclDetails x RdrName)
...
mkPatSynMatchGroup :: SourceTextX x => Located RdrName
                   -> Located (OrdList (LHsDecl x RdrName))
                   -> P (MatchGroup x RdrName (LHsExpr x RdrName))
...
recordPatSynErr :: SourceTextX x => SrcSpan -> LPat x RdrName -> P a
recordPatSynErr loc pat =
    parseErrorSDoc loc $
    text "record syntax not supported for pattern synonym declarations:" $$
    ppr pat

mkConDeclH98 :: Located RdrName -> Maybe [LHsTyVarBndr x RdrName]
                -> LHsContext x RdrName -> HsConDeclDetails x RdrName
                -> ConDecl x RdrName
...
mkGadtDecl :: [Located RdrName]
           -> LHsSigType x RdrName     -- Always a HsForAllTy
           -> ConDecl x RdrName
...
checkTyVarsP :: SourceTextX x
             => SDoc -> SDoc -> Located RdrName -> [LHsType x RdrName]
             -> P (LHsQTyVars x RdrName)
...
checkTyVars :: SourceTextX x
            => SDoc -> SDoc -> Located RdrName -> [LHsType x RdrName]
            -> Either (SrcSpan, SDoc) (LHsQTyVars x RdrName)
...
checkDatatypeContext :: SourceTextX x => Maybe (LHsContext x RdrName) -> P ()
...
checkTyClHdr :: SourceTextX x
             => Bool               -- True  <=> class header
                                   -- False <=> type header
             -> LHsType x RdrName
             -> P (Located RdrName,          -- the head symbol (type or class name)
                   [LHsType x RdrName],      -- parameters of head symbol
...
checkContext :: LHsType x RdrName -> P ([AddAnn],LHsContext x RdrName)
...
checkPattern :: SourceTextX x => SDoc -> LHsExpr x RdrName -> P (LPat x RdrName)
checkPattern msg e = checkLPat msg e

checkPatterns :: SourceTextX x
              =>  SDoc -> [LHsExpr x RdrName] -> P [LPat x RdrName]
checkPatterns msg es = mapM (checkPattern msg) es

checkLPat :: SourceTextX x => SDoc -> LHsExpr x RdrName -> P (LPat x RdrName)
checkLPat msg e@(L l _) = checkPat msg l e []

checkPat :: SourceTextX x
         => SDoc -> SrcSpan -> LHsExpr x RdrName -> [LPat x RdrName]
         -> P (LPat x RdrName)
...
checkAPat :: SourceTextX x
          => SDoc -> SrcSpan -> HsExpr x RdrName -> P (Pat x RdrName)
...
placeHolderPunRhs :: LHsExpr x RdrName
...
checkPatField :: SourceTextX x
              => SDoc -> LHsRecField RdrName (LHsExpr x RdrName)
              -> P (LHsRecField RdrName (LPat x RdrName))
checkPatField msg (L l fld) = do p <- checkLPat msg (hsRecFieldArg fld)
                                 return (L l (fld { hsRecFieldArg = p }))

patFail :: SourceTextX x => SDoc -> SrcSpan -> HsExpr x RdrName -> P a
...
checkValDef :: SourceTextX x
            => SDoc
            -> LHsExpr x RdrName
            -> Maybe (LHsType x RdrName)
            -> Located (a,GRHSs x RdrName (LHsExpr x RdrName))
            -> P ([AddAnn],HsBind x RdrName)
...
checkFunBind :: SourceTextX x
             ...
             -> [LHsExpr x RdrName]
             -> Maybe (LHsType x RdrName)
             -> Located (GRHSs x RdrName (LHsExpr x RdrName))
             -> P ([AddAnn],HsBind x RdrName)
...
makeFunBind :: Located RdrName -> [LMatch x RdrName (LHsExpr x RdrName)]
            -> HsBind x RdrName
...
checkValSigLhs :: SourceTextX x => LHsExpr x RdrName -> P (Located RdrName)
...
checkDoAndIfThenElse :: SourceTextX x
                     => LHsExpr x RdrName
                     -> Bool
                     -> LHsExpr x RdrName
                     -> Bool
...
splitBang :: LHsExpr x RdrName -> Maybe (LHsExpr x RdrName, [LHsExpr x RdrName])
...
isFunLhs :: LHsExpr x RdrName
     -> P (Maybe (Located RdrName, LexicalFixity, [LHsExpr x RdrName],[AddAnn]))
...
-- (((~a) ~b) c) ~d ==> ((~a) ~ (b c)) ~ d
splitTilde :: LHsType x RdrName -> P (LHsType x RdrName)
splitTilde t = go t
...
-- [~a, ~b, c, ~d] ==> (~a) ~ b c ~ d
splitTildeApps :: [LHsAppType x RdrName] -> P [LHsAppType x RdrName]
...
checkCommand :: SourceTextX x => LHsExpr x RdrName -> P (LHsCmd x RdrName)
checkCommand lc = locMap checkCmd lc

locMap :: (SrcSpan -> a -> P b) -> Located a -> P (Located b)
locMap f (L l a) = f l a >>= (\b -> return $ L l b)

checkCmd :: SourceTextX x => SrcSpan -> HsExpr x RdrName -> P (HsCmd x RdrName)
...
checkCmdLStmt :: SourceTextX x => ExprLStmt x RdrName -> P (CmdLStmt x RdrName)
checkCmdLStmt = locMap checkCmdStmt

checkCmdStmt :: SourceTextX x
             => SrcSpan -> ExprStmt x RdrName -> P (CmdStmt x RdrName)
...
checkCmdMatchGroup :: SourceTextX x => MatchGroup x RdrName (LHsExpr x RdrName)
                -> P (MatchGroup x RdrName (LHsCmd x RdrName))
...
checkCmdGRHSs :: SourceTextX x => GRHSs x RdrName (LHsExpr x RdrName)
           -> P (GRHSs x RdrName (LHsCmd x RdrName))
checkCmdGRHSs (GRHSs grhss binds) = do
    grhss' <- mapM checkCmdGRHS grhss
    return $ GRHSs grhss' binds

checkCmdGRHS :: SourceTextX x => LGRHS x RdrName (LHsExpr x RdrName)
             -> P (LGRHS x RdrName (LHsCmd x RdrName))
...
cmdFail :: SourceTextX x => SrcSpan -> HsExpr x RdrName -> P a
cmdFail loc e = parseErrorSDoc loc (text "Parse error in command:" <+> ppr e)
cmdStmtFail :: SourceTextX x
            => SrcSpan -> Stmt x RdrName (LHsExpr x RdrName) -> P a
...
mkRecConstrOrUpdate :: SourceTextX x 
        => LHsExpr x RdrName
        -> SrcSpan
        -> ([LHsRecField RdrName (LHsExpr x RdrName)], Bool)
        -> P (HsExpr x RdrName)
...
mkRdrRecordUpd :: LHsExpr x RdrName
               -> [LHsRecUpdField x RdrName] -> HsExpr x RdrName
...
mkRdrRecordCon :: SourceTextX x
               => Located RdrName -> HsRecordBinds x RdrName -> HsExpr x RdrName
...
mk_rec_upd_field :: HsRecField RdrName (LHsExpr x RdrName)
                 -> HsRecUpdField x RdrName
...
data SumOrTuple x
  = Sum ConTag Arity (LHsExpr x RdrName)
  | Tuple [LHsTupArg x RdrName]

mkSumOrTuple :: SourceTextX x
             => Boxity -> SrcSpan -> SumOrTuple x -> P (HsExpr x RdrName)
...
mkSumOrTuple Boxed l (Sum alt arity (L _ e)) =
  ...
  where
    ppr_boxed_sum :: SourceTextX x
                   => ConTag -> Arity -> HsExpr x RdrName -> SDoc
