...
mkClassDecl :: SrcSpan
            -> Located (Maybe (LHsContext GHCP), LHsType GHCP)
            -> Located (a,[Located (FunDep (Located (IdP GHCP)))])
            -> OrdList (LHsDecl GHCP)
            -> P (LTyClDecl GHCP)
...
mkATDefault :: LTyFamInstDecl GHCP
            -> Either (SrcSpan, SDoc) (LTyFamDefltEqn GHCP)
...
mkTyFamInstEqn :: LHsType GHCP
               -> LHsType GHCP
               -> P (TyFamInstEqn GHCP,[AddAnn])
...
mkFamDecl :: SrcSpan
          -> FamilyInfo GHCP
          -> LHsType GHCP                   -- LHS
          -> Located (FamilyResultSig GHCP) -- Optional result signature
          -> Maybe (LInjectivityAnn GHCP)   -- Injectivity annotation
          -> P (LTyClDecl GHCP)
...
mkSpliceDecl :: LHsExpr GHCP -> HsDecl GHCP
-- If the user wrote
...
mkRoleAnnotDecl :: SrcSpan
                -> Located (IdP GHCP)                -- type being annotated
                -> [Located (Maybe FastString)]      -- roles
                -> P (LRoleAnnotDecl GHCP)
...
cvTopDecls :: OrdList (LHsDecl GHCP) -> [LHsDecl GHCP]
cvTopDecls decls = go (fromOL decls)
  where
    go :: [LHsDecl GHCP] -> [LHsDecl GHCP]
    go []                   = []
...
cvBindGroup :: OrdList (LHsDecl GHCP) -> P (HsValBinds GHCP)
cvBindGroup binding
  = do { (mbs, sigs, fam_ds, tfam_insts, dfam_insts, _) <- cvBindsAndSigs binding
       ; ASSERT( null fam_ds && null tfam_insts && null dfam_insts)
         return $ ValBindsIn mbs sigs }

cvBindsAndSigs :: OrdList (LHsDecl GHCP)
  -> P (LHsBinds GHCP, [LSig GHCP], [LFamilyDecl GHCP]
          , [LTyFamInstDecl GHCP], [LDataFamInstDecl GHCP], [LDocDecl])
...
getMonoBind :: LHsBind GHCP -> [LHsDecl GHCP]
  -> (LHsBind GHCP, [LHsDecl GHCP])
...
has_args :: [LMatch GHCP (LHsExpr GHCP)] -> Bool
...
splitCon :: LHsType GHCP
      -> P (Located (IdP GHCP), HsConDeclDetails GHCP)
...
tyConToDataCon :: SrcSpan -> IdP GHCP -> P (Located (IdP GHCP))
-- See Note [Parsing data constructors is hard]
...
mkPatSynMatchGroup :: Located (IdP GHCP)
                   -> Located (OrdList (LHsDecl GHCP))
                   -> P (MatchGroup GHCP (LHsExpr GHCP))
...
recordPatSynErr :: SrcSpan -> LPat GHCP -> P a
recordPatSynErr loc pat =
    parseErrorSDoc loc $
    text "record syntax not supported for pattern synonym declarations:" $$
    ppr pat

mkConDeclH98 :: Located (IdP GHCP) -> Maybe [LHsTyVarBndr GHCP]
                -> LHsContext GHCP -> HsConDeclDetails GHCP
                -> ConDecl GHCP
...
mkGadtDecl :: [Located (IdP GHCP)]
           -> LHsSigType GHCP     -- Always a HsForAllTy
           -> ConDecl GHCP
...
checkTyVarsP :: SDoc -> SDoc -> Located (IdP GHCP) -> [LHsType GHCP] -> P (LHsQTyVars GHCP)
...
checkTyVars :: SDoc -> SDoc -> Located (IdP GHCP) -> [LHsType GHCP]
            -> Either (SrcSpan, SDoc) (LHsQTyVars GHCP)
...
checkDatatypeContext :: Maybe (LHsContext GHCP) -> P ()
...
checkTyClHdr :: Bool               -- True  <=> class header
                                   -- False <=> type header
             -> LHsType GHCP
             -> P (Located (IdP GHCP),       -- the head symbol (type or class name)
                   [LHsType GHCP],        -- parameters of head symbol
...
checkContext :: LHsType GHCP -> P ([AddAnn],LHsContext GHCP)
...
checkPattern :: SDoc -> LHsExpr GHCP -> P (LPat GHCP)
checkPattern msg e = checkLPat msg e

checkPatterns :: SDoc -> [LHsExpr GHCP] -> P [LPat GHCP]
checkPatterns msg es = mapM (checkPattern msg) es

checkLPat :: SDoc -> LHsExpr GHCP -> P (LPat GHCP)
checkLPat msg e@(L l _) = checkPat msg l e []

checkPat :: SDoc -> SrcSpan -> LHsExpr GHCP -> [LPat GHCP]
         -> P (LPat GHCP)
...
checkAPat :: SDoc -> SrcSpan -> HsExpr GHCP -> P (Pat GHCP)
...
placeHolderPunRhs :: LHsExpr GHCP
...
checkPatField :: SDoc -> LHsRecField GHCP (LHsExpr GHCP)
              -> P (LHsRecField GHCP (LPat GHCP))
checkPatField msg (L l fld) = do p <- checkLPat msg (hsRecFieldArg fld)
                                 return (L l (fld { hsRecFieldArg = p }))

patFail :: SDoc -> SrcSpan -> HsExpr GHCP -> P a
...
checkValDef :: SDoc
            -> LHsExpr GHCP
            -> Maybe (LHsType GHCP)
            -> Located (a,GRHSs GHCP (LHsExpr GHCP))
            -> P ([AddAnn],HsBind GHCP)
...
checkFunBind :: SDoc
             ...
             -> [LHsExpr GHCP]
             -> Maybe (LHsType GHCP)
             -> Located (GRHSs GHCP (LHsExpr GHCP))
             -> P ([AddAnn],HsBind GHCP)
...
makeFunBind :: Located (IdP GHCP) -> [LMatch GHCP (LHsExpr GHCP)]
            -> HsBind GHCP
...
checkValSigLhs :: LHsExpr GHCP -> P (Located (IdP GHCP))
...
checkDoAndIfThenElse :: LHsExpr GHCP
                     -> Bool
                     -> LHsExpr GHCP
                     -> Bool
                     -> LHsExpr GHCP
...
splitBang :: LHsExpr GHCP -> Maybe (LHsExpr GHCP, [LHsExpr GHCP])
...
isFunLhs :: LHsExpr GHCP
      -> P (Maybe (Located (IdP GHCP), LexicalFixity, [LHsExpr GHCP],[AddAnn]))
...
-- (((~a) ~b) c) ~d ==> ((~a) ~ (b c)) ~ d
splitTilde :: LHsType GHCP -> P (LHsType GHCP)
splitTilde t = go t
...
-- [~a, ~b, c, ~d] ==> (~a) ~ b c ~ d
splitTildeApps :: [LHsAppType GHCP] -> P [LHsAppType GHCP]
...
checkCmdLStmt :: ExprLStmt GHCP -> P (CmdLStmt GHCP)
checkCmdLStmt = locMap checkCmdStmt

checkCmdStmt :: SrcSpan -> ExprStmt GHCP -> P (CmdStmt GHCP)
...
checkCmdMatchGroup :: MatchGroup GHCP (LHsExpr GHCP) -> P (MatchGroup GHCP (LHsCmd GHCP))
...
checkCmdGRHSs :: GRHSs GHCP (LHsExpr GHCP) -> P (GRHSs GHCP (LHsCmd GHCP))
checkCmdGRHSs (GRHSs grhss binds) = do
    grhss' <- mapM checkCmdGRHS grhss
    return $ GRHSs grhss' binds

checkCmdGRHS :: LGRHS GHCP (LHsExpr GHCP) -> P (LGRHS GHCP (LHsCmd GHCP))
...
cmdFail :: SrcSpan -> HsExpr GHCP -> P a
cmdFail loc e = parseErrorSDoc loc (text "Parse error in command:" <+> ppr e)
cmdStmtFail :: SrcSpan -> Stmt GHCP (LHsExpr GHCP) -> P a
...
mkRecConstrOrUpdate
        :: LHsExpr GHCP
        -> SrcSpan
        -> ([LHsRecField GHCP (LHsExpr GHCP)], Bool)
        -> P (HsExpr GHCP)
...
mkRdrRecordUpd :: LHsExpr GHCP -> [LHsRecUpdField GHCP] -> HsExpr GHCP
...
mkRdrRecordCon :: Located (IdP GHCP) -> HsRecordBinds GHCP -> HsExpr GHCP
...
mk_rec_upd_field :: HsRecField GHCP (LHsExpr GHCP) -> HsRecUpdField GHCP
...
data ImpExpQcSpec = ImpExpQcName (Located (IdP GHCP))
                  | ImpExpQcType (Located (IdP GHCP))
                  | ImpExpQcWildcard

mkModuleImpExp :: Located ImpExpQcSpec -> ImpExpSubSpec -> P (IE GHCP)
...
checkImportSpec :: Located [LIE GHCP] -> P (Located [LIE GHCP])
...
data SumOrTuple
  = Sum ConTag Arity (LHsExpr GHCP)
  | Tuple [LHsTupArg GHCP]

mkSumOrTuple :: Boxity -> SrcSpan -> SumOrTuple -> P (HsExpr GHCP)
...
mkSumOrTuple Boxed l (Sum alt arity (L _ e)) =
  ...
  where
    ppr_boxed_sum :: ConTag -> Arity -> HsExpr GHCP -> SDoc
