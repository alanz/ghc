...
dsOverLit :: HsOverLit GHCT -> DsM CoreExpr
dsOverLit lit = do { dflags <- getDynFlags
                   ; warnAboutOverflowedLiterals dflags lit
                   ; dsOverLit' dflags lit }

dsOverLit' :: DynFlags -> HsOverLit GHCT -> DsM CoreExpr
...
warnAboutOverflowedLiterals :: DynFlags -> HsOverLit GHCT -> DsM ()
warnAboutOverflowedLiterals dflags lit
...
warnAboutEmptyEnumerations :: DynFlags -> LHsExpr GHCT -> Maybe (LHsExpr GHCT) -> LHsExpr GHCT -> DsM ()
...
getLHsIntegralLit :: LHsExpr GHCT -> Maybe (Integer, Name)
...
getIntegralLit :: HsOverLit GHCT -> Maybe (Integer, Name)
...
tidyLitPat :: HsLit GHCT -> Pat GHCT
...
----------------
tidyNPat :: (HsLit GHCT -> Pat GHCT)   -- How to tidy a LitPat
         ...
         -> HsOverLit GHCT -> Maybe (SyntaxExpr GHCT) -> SyntaxExpr GHCT -> Type
         -> Pat GHCT
tidyNPat tidy_lit_pat (OverLit val False _ ty) mb_neg _eq outer_ty
  ...
  where
    ...
    mk_con_pat :: DataCon -> HsLit GHCT -> Pat GHCT
...
matchLiterals (var:vars) ty sub_groups
  ...
  where
    ...
    wrap_str_guard :: IdP GHCT -> (Literal,MatchResult) -> DsM MatchResult
...
---------------------------
hsLitKey :: DynFlags -> HsLit GHCT -> Literal
...
matchNPats :: [IdP GHCT] -> Type -> [EquationInfo] -> DsM MatchResult
...
matchNPlusKPats :: [IdP GHCT] -> Type -> [EquationInfo] -> DsM MatchResult
