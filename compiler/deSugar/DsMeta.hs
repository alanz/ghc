...
repE (ExplicitList _ _ _ es) = do { xs <- repLEs es; repListExp xs }
repE e@(ExplicitPArr _ _ _) = notHandled "Parallel arrays" (ppr e)
...
repE (ExplicitSum _ alt arity e _)
 = do { e1 <- repLE e
...
repE (HsStatic _ _ e)      = repLE e >>= rep2 staticEName . (:[]) . unC
