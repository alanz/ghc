...
-----------------------------------------------------------------------------
dsBracket :: HsBracket GHCR -> [PendingTcSplice] -> DsM CoreExpr
...
repTopP :: LPat GHCR -> DsM (Core TH.PatQ)
repTopP pat = do { ss <- mkGenSyms (collectPatBinders pat)
                 ; pat' <- addBinds ss (repLP pat)
                 ; wrapGenSyms ss pat' }

repTopDs :: HsGroup GHCR -> DsM (Core (TH.Q [TH.Dec]))
...
hsSigTvBinders :: HsValBinds GHCR -> [IdP GHCR]
-- See Note [Scoped type variables in bindings]
hsSigTvBinders binds
  = concatMap get_scoped_tvs sigs
  where
    get_scoped_tvs :: LSig GHCR -> [IdP GHCR]
...
--
repTyClD :: LTyClDecl GHCR -> DsM (Maybe (SrcSpan, Core TH.DecQ))
...
-------------------------
repRoleD :: LRoleAnnotDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
...
repFamilyDecl :: LFamilyDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
repFamilyDecl decl@(L loc (FamilyDecl { fdInfo      = info,
  ...
  = do { tc1 <- lookupLOcc tc           -- See note [Binders and occurrences]
       ; let mkHsQTvs :: [LHsTyVarBndr GHCR] -> LHsQTyVars GHCR
...
repFamilyResultSig :: FamilyResultSig GHCR -> DsM (Core TH.FamilyResultSig)
...
-- result variable.
repFamilyResultSigToMaybeKind :: FamilyResultSig GHCR
...
repInjectivityAnn :: Maybe (LInjectivityAnn GHCR)
...
repFamilyDecls :: [LFamilyDecl GHCR] -> DsM [Core TH.DecQ]
repFamilyDecls fds = liftM de_loc (mapM repFamilyDecl fds)

repAssocTyFamDefaults :: [LTyFamDefltEqn GHCR] -> DsM [Core TH.DecQ]
repAssocTyFamDefaults = mapM rep_deflt
  where
     -- very like repTyFamEqn, but different in the details
    rep_deflt :: LTyFamDefltEqn GHCR -> DsM (Core TH.DecQ)
...
-- represent fundeps
--
repLFunDeps :: [Located (FunDep (Located (IdP GHCR)))] -> DsM (Core [TH.FunDep])
repLFunDeps fds = repList funDepTyConName repLFunDep fds

repLFunDep :: Located (FunDep (Located (IdP GHCR))) -> DsM (Core TH.FunDep)
...
--
repInstD :: LInstDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
...
repClsInstD :: ClsInstDecl GHCR -> DsM (Core TH.DecQ)
...
repStandaloneDerivD :: LDerivDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
...
repTyFamInstD :: TyFamInstDecl GHCR -> DsM (Core TH.DecQ)
...
repTyFamEqn :: LTyFamInstEqn GHCR -> DsM (Core TH.TySynEqnQ)
...
repDataFamInstD :: DataFamInstDecl GHCR -> DsM (Core TH.DecQ)
...
repForD :: Located (ForeignDecl GHCR) -> DsM (SrcSpan, Core TH.DecQ)
...
repFixD :: LFixitySig GHCR -> DsM [(SrcSpan, Core TH.DecQ)]
...
repRuleD :: LRuleDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
...
repAnnD :: LAnnDecl GHCR -> DsM (SrcSpan, Core TH.DecQ)
...
repAnnProv :: AnnProvenance (IdP GHCR) -> DsM (Core TH.AnnTarget)
...
repC :: LConDecl GHCR -> DsM (Core TH.ConQ)
...
repBangTy :: LBangType GHCR -> DsM (Core (TH.BangTypeQ))
repBangTy ty = do
...
repDerivs :: HsDeriving GHCR -> DsM (Core [TH.DerivClauseQ])
repDerivs (L _ clauses) = repList derivClauseQTyConName repDerivClause clauses

repDerivClause :: LHsDerivingClause GHCR
...
repDerivClause (L _ (HsDerivingClause { deriv_clause_strategy = dcs
  ...
  where
    rep_deriv_ty :: LHsType GHCR -> DsM (Core TH.TypeQ)
...
rep_sigs :: [LSig GHCR] -> DsM [Core TH.DecQ]
rep_sigs sigs = do locs_cores <- rep_sigs' sigs
                   return $ de_loc $ sort_by_loc locs_cores

rep_sigs' :: [LSig GHCR] -> DsM [(SrcSpan, Core TH.DecQ)]
        -- We silently ignore ones we don't recognise
rep_sigs' = concatMapM rep_sig

rep_sig :: LSig GHCR -> DsM [(SrcSpan, Core TH.DecQ)]
...
rep_ty_sig :: IdP GHCR -> SrcSpan -> LHsSigType GHCR -> Located (IdP GHCR)
...
rep_patsyn_ty_sig :: SrcSpan -> LHsSigType GHCR -> Located (IdP GHCR)
                  -> DsM (SrcSpan, Core TH.DecQ)
...
rep_wc_ty_sig :: IdP GHCR -> SrcSpan -> LHsSigWcType GHCR -> Located (IdP GHCR)
              -> DsM (SrcSpan, Core TH.DecQ)
...
rep_specialise :: Located (IdP GHCR) -> LHsSigType GHCR -> InlinePragma
               -> SrcSpan
               -> DsM [(SrcSpan, Core TH.DecQ)]
...
rep_specialiseInst :: LHsSigType GHCR -> SrcSpan -> DsM [(SrcSpan, Core TH.DecQ)]
...
rep_complete_sig :: Located [Located (IdP GHCR)]
                 -> Maybe (Located (IdP GHCR))
                 -> SrcSpan
                 -> DsM [(SrcSpan, Core TH.DecQ)]
...
addSimpleTyVarBinds :: [IdP GHCR]                -- the binders to be added
...
addTyVarBinds :: LHsQTyVars GHCR                            -- the binders to be added
...
addTyClTyVarBinds :: LHsQTyVars GHCR
...
--
repTyVarBndrWithKind :: LHsTyVarBndr GHCR
...
repTyVarBndr :: LHsTyVarBndr GHCR -> DsM (Core TH.TyVarBndr)
...
--
repLContext :: LHsContext GHCR -> DsM (Core TH.CxtQ)
repLContext (L _ ctxt) = repContext ctxt

repContext :: HsContext GHCR -> DsM (Core TH.CxtQ)
repContext ctxt = do preds <- repList typeQTyConName repLTy ctxt
                     repCtxt preds

repHsSigType :: LHsSigType GHCR -> DsM (Core TH.TypeQ)
...
repHsPatSynSigType :: LHsSigType GHCR -> DsM (Core TH.TypeQ)
...
repHsSigWcType :: LHsSigWcType GHCR -> DsM (Core TH.TypeQ)
repHsSigWcType (HsWC { hswc_body = sig1 })
  = repHsSigType sig1

-- yield the representation of a list of types
repLTys :: [LHsType GHCR] -> DsM [Core TH.TypeQ]
repLTys tys = mapM repLTy tys

-- represent a type
repLTy :: LHsType GHCR -> DsM (Core TH.TypeQ)
repLTy (L _ ty) = repTy ty

repForall :: HsType GHCR -> DsM (Core TH.TypeQ)
...
repTy :: HsType GHCR -> DsM (Core TH.TypeQ)
...
-- represent a kind
--
repLKind :: LHsKind GHCR -> DsM (Core TH.Kind)
repLKind ki
...
repMaybeLKind :: Maybe (LHsKind GHCR)
              -> DsM (Core (Maybe TH.Kind))
...
repNonArrowLKind :: LHsKind GHCR -> DsM (Core TH.Kind)
repNonArrowLKind (L _ ki) = repNonArrowKind ki

repNonArrowKind :: HsKind GHCR -> DsM (Core TH.Kind)
...
repSplice :: HsSplice GHCR -> DsM (Core a)
...
rep_splice :: IdP GHCR -> DsM (Core a)
...
repLEs :: [LHsExpr GHCR] -> DsM (Core [TH.ExpQ])
repLEs es = repList expQTyConName repLE es

-- FIXME: some of these panics should be converted into proper error messages
--        unless we can make sure that constructs, which are plainly not
--        supported in TH already lead to error messages at an earlier stage
repLE :: LHsExpr GHCR -> DsM (Core TH.ExpQ)
repLE (L loc e) = putSrcSpanDs loc (repE e)

repE :: HsExpr GHCR -> DsM (Core TH.ExpQ)
repE (HsVar (L _ x))            =
...
repMatchTup ::  LMatch GHCR (LHsExpr GHCR) -> DsM (Core TH.MatchQ)
...
repClauseTup ::  LMatch GHCR (LHsExpr GHCR) -> DsM (Core TH.ClauseQ)
...
repGuards ::  [LGRHS GHCR (LHsExpr GHCR)] ->  DsM (Core TH.BodyQ)
...
repLGRHS :: LGRHS GHCR (LHsExpr GHCR) -> DsM ([GenSymBind], (Core (TH.Q (TH.Guard, TH.Exp))))
...
repFields :: HsRecordBinds GHCR -> DsM (Core [TH.Q TH.FieldExp])
repFields (HsRecFields { rec_flds = flds })
  = repList fieldExpQTyConName rep_fld flds
  where
    rep_fld :: LHsRecField GHCR (LHsExpr GHCR) -> DsM (Core (TH.Q TH.FieldExp))
    rep_fld (L _ fld) = do { fn <- lookupLOcc (hsRecFieldSel fld)
                           ; e  <- repLE (hsRecFieldArg fld)
                           ; repFieldExp fn e }

repUpdFields :: [LHsRecUpdField GHCR] -> DsM (Core [TH.Q TH.FieldExp])
repUpdFields = repList fieldExpQTyConName rep_fld
  where
    rep_fld :: LHsRecUpdField GHCR -> DsM (Core (TH.Q TH.FieldExp))
...
repLSts :: [LStmt GHCR (LHsExpr GHCR)] -> DsM ([GenSymBind], [Core TH.StmtQ])
repLSts stmts = repSts (map unLoc stmts)

repSts :: [Stmt GHCR (LHsExpr GHCR)] -> DsM ([GenSymBind], [Core TH.StmtQ])
...
repSts (ParStmt stmt_blocks _ _ _ : ss) =
   do { (ss_s, stmt_blocks1) <- mapAndUnzipM rep_stmt_block stmt_blocks
      ...
      ; return (ss1++ss2, z : zs) }
   where
     rep_stmt_block :: ParStmtBlock GHCR GHCR -> DsM ([GenSymBind], Core [TH.StmtQ])
...
repBinds :: HsLocalBinds GHCR -> DsM ([GenSymBind], Core [TH.DecQ])
...
rep_val_binds :: HsValBinds GHCR -> DsM [(SrcSpan, Core TH.DecQ)]
...
rep_binds :: LHsBinds GHCR -> DsM [Core TH.DecQ]
rep_binds binds = do { binds_w_locs <- rep_binds' binds
                     ; return (de_loc (sort_by_loc binds_w_locs)) }

rep_binds' :: LHsBinds GHCR -> DsM [(SrcSpan, Core TH.DecQ)]
rep_binds' = mapM rep_bind . bagToList

rep_bind :: LHsBind GHCR -> DsM (SrcSpan, Core TH.DecQ)
-- Assumes: all the binders of the binding are already in the meta-env
...
rep_bind (L loc (PatSynBind (PSB { psb_id   = syn
  ...
  = do { syn'      <- lookupLBinder syn
       ...
       ; return (loc, patSynD'') }
  where
    mkGenArgSyms :: HsPatSynDetails (Located (IdP GHCR)) -> DsM [GenSymBind]
    ...
    wrapGenArgSyms :: HsPatSynDetails (Located (IdP GHCR))
...
repPatSynArgs :: HsPatSynDetails (Located (IdP GHCR)) -> DsM (Core TH.PatSynArgsQ)
...
repPatSynDir :: HsPatSynDir GHCR -> DsM (Core TH.PatSynDirQ)
...
repLambda :: LMatch GHCR (LHsExpr GHCR) -> DsM (Core TH.ExpQ)
...
repLPs :: [LPat GHCR] -> DsM (Core [TH.PatQ])
repLPs ps = repList patQTyConName repLP ps

repLP :: LPat GHCR -> DsM (Core TH.PatQ)
repLP (L _ p) = repP p

repP :: Pat GHCR -> DsM (Core TH.PatQ)
...
repP (ConPatIn dc details)
 ...
 where
   rep_fld :: LHsRecField GHCR (LPat GHCR) -> DsM (Core (TH.Name,TH.PatQ))
...
mkGenSyms :: [IdP GHCR] -> DsM [GenSymBind]
...
--
lookupLBinder :: Located (IdP GHCR) -> DsM (Core TH.Name)
lookupLBinder (L _ n) = lookupBinder n

lookupBinder :: IdP GHCR -> DsM (Core TH.Name)
...
--
lookupLOcc :: Located (IdP GHCR) -> DsM (Core TH.Name)
-- Lookup an occurrence; it can't be a splice.
-- Use the in-scope bindings if they exist
lookupLOcc (L _ n) = lookupOcc n

lookupOcc :: IdP GHCR -> DsM (Core TH.Name)
lookupOcc n
...
globalVar :: IdP GHCR -> DsM (Core TH.Name)
...
lookupType :: IdP GHCR  -- Name of type constructor (e.g. TH.ExpQ)
           -> DsM Type  -- The type
...
nameLit :: IdP GHCR -> DsM (Core String)
...
rep2 :: IdP GHCR -> [ CoreExpr ] -> DsM (Core a)
rep2 n xs = do { id <- dsLookupGlobalId n
               ; return (MkC (foldl App (Var id) xs)) }

dataCon' :: IdP GHCR -> [CoreExpr] -> DsM (Core a)
dataCon' n args = do { id <- dsLookupDataCon n
                     ; return $ MkC $ mkCoreConApps id args }

dataCon :: IdP GHCR -> DsM (Core a)
dataCon n = dataCon' n []
...
--------------- Expressions -----------------
repVarOrCon :: IdP GHCR -> Core TH.Name -> DsM (Core TH.ExpQ)
...
------------ Guards ----
repLNormalGE :: LHsExpr GHCR -> LHsExpr GHCR -> DsM (Core (TH.Q (TH.Guard, TH.Exp)))
...
repProto :: IdP GHCR -> Core TH.Name -> Core TH.TypeQ -> DsM (Core TH.DecQ)
repProto mk_sig (MkC s) (MkC ty) = rep2 mk_sig [s, ty]

repCtxt :: Core [TH.PredQ] -> DsM (Core TH.CxtQ)
repCtxt (MkC tys) = rep2 cxtName [tys]

repDataCon :: Located (IdP GHCR)
           -> HsConDeclDetails GHCR
           -> DsM (Core TH.ConQ)
repDataCon con details
    = do con' <- lookupLOcc con -- See Note [Binders and occurrences]
         repConstr details Nothing [con']

repGadtDataCons :: [Located (IdP GHCR)]
                -> HsConDeclDetails GHCR
                -> LHsType GHCR
                -> DsM (Core TH.ConQ)
...
repConstr :: HsConDeclDetails GHCR
          -> Maybe (LHsType GHCR)
          -> [Core TH.Name]
          -> DsM (Core TH.ConQ)
...
repConstr (RecCon (L _ ips)) resTy cons
    ...
    where
      ...
      rep_one_ip :: LBangType GHCR -> LFieldOcc GHCR -> DsM (Core a)
...
repLiteral :: HsLit GHCR -> DsM (Core TH.Lit)
...
mk_integer :: Integer -> DsM (HsLit GHCR)
mk_integer  i = do integer_ty <- lookupType integerTyConName
                   return $ HsInteger noSourceText i integer_ty

mk_rational :: FractionalLit -> DsM (HsLit GHCR)
mk_rational r = do rat_ty <- lookupType rationalTyConName
                   return $ HsRat def r rat_ty
mk_string :: FastString -> DsM (HsLit GHCR)
mk_string s = return $ HsString noSourceText s

mk_char :: Char -> DsM (HsLit GHCR)
mk_char c = return $ HsChar noSourceText c

repOverloadedLiteral :: HsOverLit GHCR -> DsM (Core TH.Lit)
...
mk_lit :: OverLitVal -> DsM (HsLit GHCR)
...
repList :: IdP GHCR -> (a  -> DsM (Core b))
                    -> [a] -> DsM (Core [b])
repList tc_name f args
  = do { args1 <- mapM f args
       ; coreList tc_name args1 }

coreList :: IdP GHCR    -- Of the TyCon of the element type
         -> [Core a] -> DsM (Core [a])
coreList tc_name es
...
coreNothing :: IdP GHCR    -- ^ Name of the TyCon of the element type
            -> DsM (Core (Maybe a))
coreNothing tc_name =
...
coreJust :: IdP GHCR    -- ^ Name of the TyCon of the element type
