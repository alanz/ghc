...
rep_specialiseInst :: LHsSigType GhcRn -> SrcSpan -> DsM [(SrcSpan, Core TH.DecQ)]
...
repLGRHS :: LGRHS GhcRn (LHsExpr GhcRn) -> DsM ([GenSymBind], (Core (TH.Q (TH.Guard, TH.Exp))))
...
repFields (HsRecFields { rec_flds = flds })
  ...
  where
    rep_fld :: LHsRecField GhcRn (LHsExpr GhcRn) -> DsM (Core (TH.Q TH.FieldExp))
...
repSts (ParStmt stmt_blocks _ _ _ : ss) =
   do { (ss_s, stmt_blocks1) <- mapAndUnzipM rep_stmt_block stmt_blocks
      ...
      ; return (ss1++ss2, z : zs) }
   where
     rep_stmt_block :: ParStmtBlock GhcRn GhcRn -> DsM ([GenSymBind], Core [TH.StmtQ])
...
------------ Guards ----
repLNormalGE :: LHsExpr GhcRn -> LHsExpr GhcRn -> DsM (Core (TH.Q (TH.Guard, TH.Exp)))
