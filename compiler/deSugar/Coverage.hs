...
addTickHsExpr (OpApp x e1 e2 fix e3) =
        liftM4 (OpApp x)
                (addTickLHsExpr e1)
...
addTickHsExpr (ExplicitTuple x es boxity) =
        liftM2 (ExplicitTuple x)
                ...
                (return boxity)
addTickHsExpr (ExplicitSum x tag arity e ty) = do
        e' <- addTickLHsExpr e
        return (ExplicitSum x tag arity e' ty)
...
addTickHsExpr (HsMultiIf x ty alts)
...
addTickHsExpr (HsDo x cxt (L l stmts) srcloc)
  = do { (stmts', _) <- addTickLStmts' forQual stmts (return ())
       ; return (HsDo x cxt (L l stmts') srcloc) }
  where
        forQual = case cxt of
                    ListComp -> Just $ BinBox QualBinBox
                    _        -> Nothing
addTickHsExpr (ExplicitList x ty wit es) =
        liftM3 (ExplicitList x)
                (return ty)
                (addTickWit wit)
             ...
             where addTickWit Nothing = return Nothing
                   addTickWit (Just fln)
                     = do fln' <- addTickSyntaxExpr hpcSrcSpan fln
                          return (Just fln')
addTickHsExpr (ExplicitPArr x ty es) =
        liftM2 (ExplicitPArr x)
                (return ty)
                (mapM (addTickLHsExpr) es)

addTickHsExpr (HsStatic x fvs e) = HsStatic x fvs <$> addTickLHsExpr e
...
addTickHsExpr (ArithSeq  x ty wit arith_seq) =
        liftM3 (ArithSeq x)
                (return ty)
                (addTickWit wit)
...
addTickHsExpr (HsTickPragma _ _ _ _ (L pos e0)) = do
    e2 <- allocTickBox (ExpBox False) False False pos $
                addTickHsExpr e0
    return $ unLoc e2
addTickHsExpr (PArrSeq x ty arith_seq) =
        liftM2 (PArrSeq x)
                (return ty)
                (addTickArithSeqInfo arith_seq)
