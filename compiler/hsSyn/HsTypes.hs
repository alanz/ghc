...
splitLHsInstDeclTy_maybe inst_ty = do
    ...
    pprTrace "splitLHsInstDeclTy_maybe:(tvs,cxt,ty,inst_ty)=" (ppr (tvs,cxt,ty,inst_ty)) (return ()) -- ++AZ++
    (cls, tys) <- splitLHsClassTy_maybe ty
    pprTrace "splitLHsInstDeclTy_maybe:(cls,tys)=" (ppr (cls,tys)) (return ()) -- ++AZ++
...
splitLHsForAllTy poly_ty
  = pprTrace "splitLHsForAllTy:(poly_ty)=" (ppr (poly_ty)) $
    case unLoc poly_ty of
        HsParTy ty                -> splitLHsForAllTy ty
        HsForAllTy exp extra tvs cxt (L _ (HsParTy ty)) -> splitLHsForAllTy (noLoc $ HsForAllTy exp extra tvs cxt ty)
        HsForAllTy Explicit _ tvs (L _ []) (L _ (HsForAllTy _ _ _ cxt ty)) -> (tvs, unLoc cxt, ty)
...
splitLHsClassTy_maybe ty
  ...
  where
    checkl (L l ty) args = case ty of
        ...
        -- HsForAllTy _ _ _ _ ty -> checkl ty args -- AZ: needs to be here
        HsForAllTy Implicit e tvs c (L _ (HsParTy ty))      -> checkl (L l (HsForAllTy Implicit e tvs c ty)) args
        HsForAllTy Implicit _ _vs _ ty@(L _ (HsForAllTy{})) -> checkl ty args
        _                  -> Nothing
