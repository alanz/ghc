...
mkHsDo ctxt stmts = HsDo noExt ctxt (mkLocatedList stmts) placeHolderType
...
mkHsOpApp :: LHsExpr (GhcPass id) -> IdP (GhcPass id) -> LHsExpr (GhcPass id)
          -> HsExpr (GhcPass id)
mkHsOpApp e1 op e2 = OpApp noExt e1 (noLoc (HsVar noExt (noLoc op)))
...
nlHsOpApp :: LHsExpr (GhcPass id) -> IdP (GhcPass id)
...
nlList exprs          = noLoc (ExplicitList noExt placeHolderType Nothing exprs)
