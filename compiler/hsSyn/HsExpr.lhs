...
data HsExpr id
  ...
  | HsLamCase (TypeAnnot id) (MatchGroup id (LHsExpr id)) -- ^ Lambda-case
  ...
  | HsDo        (HsStmtContext Name) -- The parameterisation is unimportant
                ...
                (TypeAnnot id)       -- Type of the whole expression

  -- | Syntactic list: [a,b,c,...]
  | ExplicitList
                (TypeAnnot id)          -- Gives type of components of list
                (Maybe (SyntaxExpr id)) -- For OverloadedLists, the fromListN witness
                [LHsExpr id]

  -- | Syntactic parallel array: [:e1, ..., en:]
  | ExplicitPArr
                (TypeAnnot id)     -- type of elements of the parallel array
                [LHsExpr id]
...
--                                 -- a match that does the job
                ...
                [TypeAnnot id]     -- Argument types of *input* record type
                [TypeAnnot id]     --              and  *output* record type
  ...
  | HsArrApp            -- Arrow tail, or arrow application (f -< arg)
        ...
        (TypeAnnot id)  -- type of the arrow expressions f,
  ...
  deriving (Typeable)
deriving instance (Data id, Data (TypeAnnot id)) => Data (HsExpr id)
...
data HsCmd id
  ...
  deriving (Typeable)
deriving instance (Data id, Data (TypeAnnot id)) => Data (HsCmd id)
...
data MatchGroup id body
  = MG { mg_alts    :: [LMatch id body]  -- The alternatives
       , mg_arg_tys :: [TypeAnnot id]    -- Types of the arguments, t1..tn
       , mg_res_ty  :: (TypeAnnot id)    -- Type of the result, tr
       , mg_origin  :: Origin }
     -- The type is the type of the entire group
     --      t1 -> ... -> tn -> tr
     -- where there are n patterns
  deriving (Typeable)
deriving instance (Data id, Data body, Data (TypeAnnot id))
   => Data (MatchGroup id body)
...
data HsSplice id  = HsSplice            --  $z  or $(f 4)
  ...
  deriving (Typeable)
deriving instance (Data id, Data (TypeAnnot id)) => Data (HsSplice id)
...
data HsBracket id = ExpBr (LHsExpr id)   -- [|  expr  |]
  ...
  deriving (Typeable)
deriving instance (Data id, Data (TypeAnnot id)) => Data (HsBracket id)
...
data ArithSeqInfo id
  ...
  | FromThenTo      (LHsExpr id)
                    ...
                    (LHsExpr id)
  deriving (Typeable)
deriving instance (Data id, Data (TypeAnnot id)) => Data (ArithSeqInfo id)
\end{code}
