  ...
  where
    ...
    cvt (MultiIfE alts)
      ...
      | otherwise      = do { alts' <- mapM cvtpair alts
                            ; return $ HsMultiIf noExt placeHolderType alts' }
    ...
    cvt (ArithSeqE dd) = do { dd' <- cvtDD dd
                            ; return $ ArithSeq noExt noPostTcExpr Nothing dd' }
    cvt (ListE xs)
      ...
      | otherwise       = do { xs' <- mapM cvtl xs
                             ; return
                               $ ExplicitList noExt placeHolderType Nothing xs'
                             }
    ...
    cvt (StaticE e)      = fmap (HsStatic noExt placeHolderNames) $ cvtl e
...
cvtOpApp x op y
  = do { op' <- cvtl op
       ; y' <- cvtl y
       ; return (OpApp noExt x op' undefined y') }
