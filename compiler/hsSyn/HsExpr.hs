...
data HsExpr p
  ...
  -- For details on above see note [Api annotations] in ApiAnnotation
  | HsMultiIf   (XMultiIf p) (PostTc p Type) [LGRHS p (LHsExpr p)]
...
type instance XOpApp         (GhcPass _) = PlaceHolder
...
type instance XExplicitSum   (GhcPass _) = PlaceHolder
type instance XCase          (GhcPass _) = PlaceHolder
type instance XIf            (GhcPass _) = PlaceHolder
type instance XMultiIf       (GhcPass _) = PlaceHolder
type instance XLet           (GhcPass _) = PlaceHolder
type instance XDo            (GhcPass _) = PlaceHolder
type instance XExplicitList  (GhcPass _) = PlaceHolder
type instance XExplicitPArr  (GhcPass _) = PlaceHolder
type instance XRecordCon     (GhcPass _) = PlaceHolder
type instance XRecordUpd     (GhcPass _) = PlaceHolder
...
type instance XArithSeq      (GhcPass _) = PlaceHolder
type instance XPArrSeq       (GhcPass _) = PlaceHolder
type instance XSCC           (GhcPass _) = PlaceHolder
type instance XCoreAnn       (GhcPass _) = PlaceHolder

type instance XBracket       GhcPs = PlaceHolder
type instance XBracket       GhcRn = [PendingRnSplice]
type instance XBracket       GhcTc = [PendingTcSplice]

type instance XSpliceE       (GhcPass _) = PlaceHolder
type instance XProc          (GhcPass _) = PlaceHolder
type instance XStatic        (GhcPass _) = PlaceHolder
type instance XArrApp        (GhcPass _) = PlaceHolder
...
ppr_expr (ExplicitSum _ alt arity expr _)
...
ppr_expr (HsDo _ do_or_list_comp (L _ stmts) _) = pprDo do_or_list_comp stmts

ppr_expr (ExplicitList _ _ _ exprs)
  = brackets (pprDeeperList fsep (punctuate comma (map ppr_lexpr exprs)))

ppr_expr (ExplicitPArr _ _ exprs)
...
ppr_expr (ArithSeq _ _ _ info) = brackets (ppr info)
ppr_expr (PArrSeq  _ _ info) = paBrackets (ppr info)
...
ppr_expr (HsArrApp _ arrow arg _ HsFirstOrderApp True)
  = hsep [ppr_lexpr arrow, larrowt, ppr_lexpr arg]
ppr_expr (HsArrApp _ arrow arg _ HsFirstOrderApp False)
  = hsep [ppr_lexpr arg, arrowt, ppr_lexpr arrow]
ppr_expr (HsArrApp _ arrow arg _ HsHigherOrderApp True)
  = hsep [ppr_lexpr arrow, larrowtt, ppr_lexpr arg]
ppr_expr (HsArrApp _ arrow arg _ HsHigherOrderApp False)
...
hsExprNeedsParens (HsDo _ sc _ _)
