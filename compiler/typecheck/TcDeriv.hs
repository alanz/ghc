...
data DerivInfo = DerivInfo { di_rep_tc  :: TyCon
                           ...
                           , di_clauses :: [LHsDerivingClause GHCR]
                           , di_ctxt    :: SDoc -- ^ error context
                           }

-- | Extract `deriving` clauses of proper data type (skips data families)
mkDerivInfos :: [LTyClDecl GHCR] -> TcM [DerivInfo]
...
tcDeriving  :: [DerivInfo]       -- All `deriving` clauses
            -> [LDerivDecl GHCR] -- All stand-alone deriving declarations
            -> TcM (TcGblEnv, Bag (InstInfo GHCR), HsValBinds GHCR)
tcDeriving deriv_infos deriv_decls
  ...
  where
    ddump_deriving :: Bag (InstInfo GHCR) -> HsValBinds GHCR
    ...
    -- See Note [Staging of tcDeriving]
    apply_inst_infos :: [ThetaType -> TcM (InstInfo GHCP)]
                     -> [DerivSpec ThetaType] -> TcM [InstInfo GHCP]
...
renameDeriv :: Bool
            ...
            -> TcM (Bag (InstInfo GHCR), HsValBinds GHCR, DefUses)
renameDeriv is_boot inst_infos bagBinds
  ...
  where
    rn_inst_info :: InstInfo GHCP -> TcM (InstInfo GHCR, FreeVars)
    rn_inst_info
...
deriveDerivInfo (DerivInfo { di_rep_tc = rep_tc, di_clauses = clauses
  ...
  where
    ...
    deriveForClause :: HsDerivingClause GHCR -> TcM [EarlyDerivSpec]
...
------------------------------------------------------------------
deriveStandalone :: LDerivDecl GHCR -> TcM [EarlyDerivSpec]
...
deriveTyData :: [TyVar] -> TyCon -> [Type]   -- LHS of data or data instance
             ...
             -> LHsSigType GHCR              -- The deriving predicate
             -> TcM [EarlyDerivSpec]
...
genInst :: DerivSpec theta
        -> TcM (ThetaType -> TcM (InstInfo GHCP), BagDerivStuff, Maybe (IdP GHCR))
...
nonUnaryErr :: LHsSigType GHCR -> SDoc
...
standaloneCtxt :: LHsSigType GHCR -> SDoc
