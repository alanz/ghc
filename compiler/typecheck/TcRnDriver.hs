...
tcRnImports :: HscEnv -> [LImportDecl GHCP] -> TcM TcGblEnv
...
tcRnHsBootDecls :: HscSource -> [LHsDecl GHCP] -> TcM TcGblEnv
...
rnTopSrcDecls :: HsGroup GHCP -> TcM (TcGblEnv, HsGroup GHCR)
...
tcTopSrcDecls :: HsGroup GHCR -> TcM (TcGblEnv, TcLclEnv)
...
tcPreludeClashWarn warnFlag name = do
  ...
  where
    ...
    importedViaPrelude :: IdP GHCR
                       -> [ImportDecl GHCR]
                       -> Bool
    importedViaPrelude name = any importViaPrelude
      where
        isPrelude :: ImportDecl GHCR -> Bool
        isPrelude imp = unLoc (ideclName imp) == pRELUDE_NAME

        -- Implicit (Prelude) import?
        isImplicit :: ImportDecl GHCR -> Bool
        isImplicit = ideclImplicit

        -- Unqualified import?
        isUnqualified :: ImportDecl GHCR -> Bool
        ...
        importViaPrelude :: ImportDecl GHCR -> Bool
...
---------------------------
tcTyClsInstDecls :: [TyClGroup GHCR]
                 -> [LDerivDecl GHCR]
                 -> [(RecFlag, LHsBinds GHCR)]
                 -> TcM (TcGblEnv,            -- The full inst env
                         [InstInfo GHCR],     -- Source-code instance decls to process;
                                              -- contains all dfuns for this module
                          HsValBinds GHCR)    -- Supporting bindings for derived instances
...
-- values, coerced to ().
tcRnStmt :: HscEnv -> GhciLStmt GHCP
         -> IO (Messages, Maybe ([IdP GHCT], LHsExpr GHCT, FixityEnv))
...
type PlanResult = ([IdP GHCT], LHsExpr GHCT)
...
tcUserStmt :: GhciLStmt GHCP -> TcM (PlanResult, FixityEnv)
...
tcGhciStmts :: [GhciLStmt GHCR] -> TcM PlanResult
tcGhciStmts stmts
...
getGhciStepIO :: TcM (LHsExpr GHCR)
getGhciStepIO = do
    ghciTy <- getGHCiMonad
