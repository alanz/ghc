...
tcDoStmts ListComp (L l stmts) res_ty
  = do  { res_ty <- expTypeToType res_ty
        ...
        ; return $ mkHsWrapCo co (HsDo noExt ListComp (L l stmts') list_ty) }
...
tcDoStmts PArrComp (L l stmts) res_ty
  = do  { res_ty <- expTypeToType res_ty
        ...
        ; stmts' <- tcStmts PArrComp (tcLcStmt parrTyCon) stmts
                            (mkCheckExpType elt_ty)
        ; return $ mkHsWrapCo co (HsDo noExt PArrComp (L l stmts') parr_ty) }

tcDoStmts DoExpr (L l stmts) res_ty
  = do  { stmts' <- tcStmts DoExpr tcDoStmt stmts res_ty
        ; res_ty <- readExpType res_ty
        ; return (HsDo noExt DoExpr (L l stmts') res_ty) }

tcDoStmts MDoExpr (L l stmts) res_ty
  = do  { stmts' <- tcStmts MDoExpr tcDoStmt stmts res_ty
        ; res_ty <- readExpType res_ty
        ; return (HsDo noExt MDoExpr (L l stmts') res_ty) }

tcDoStmts MonadComp (L l stmts) res_ty
  = do  { stmts' <- tcStmts MonadComp tcMcStmt stmts res_ty
        ; res_ty <- readExpType res_ty
        ; return (HsDo noExt MonadComp (L l stmts') res_ty) }
