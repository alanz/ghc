...
zonkExpr env (HsBracket bs body)
  = do bs' <- mapM zonk_b bs
       return (HsBracket bs' body)
  where
...
zonkExpr env (OpApp x e1 op fixity e2)
  = do new_e1 <- zonkLExpr env e1
       new_op <- zonkLExpr env op
       new_e2 <- zonkLExpr env e2
       return (OpApp x new_e1 new_op fixity new_e2)
...
zonkExpr env (ExplicitSum x alt arity expr args)
  = do new_args <- mapM (zonkTcTypeToType env) args
       new_expr <- zonkLExpr env expr
       return (ExplicitSum x alt arity new_expr new_args)
...
zonkExpr env (HsMultiIf x ty alts)
  = do { alts' <- mapM (wrapLocM zonk_alt) alts
       ; ty'   <- zonkTcTypeToType env ty
       ; return $ HsMultiIf x ty' alts' }
...
zonkExpr env (HsDo x do_or_lc (L l stmts) ty)
  = do (_, new_stmts) <- zonkStmts env zonkLExpr stmts
       new_ty <- zonkTcTypeToType env ty
       return (HsDo x do_or_lc (L l new_stmts) new_ty)

zonkExpr env (ExplicitList x ty wit exprs)
  = do (env1, new_wit) <- zonkWit env wit
       new_ty <- zonkTcTypeToType env1 ty
       new_exprs <- zonkLExprs env1 exprs
       return (ExplicitList x new_ty new_wit new_exprs)
   where zonkWit env Nothing    = return (env, Nothing)
         zonkWit env (Just fln) = second Just <$> zonkSyntaxExpr env fln

zonkExpr env (ExplicitPArr x ty exprs)
  = do new_ty <- zonkTcTypeToType env ty
       new_exprs <- zonkLExprs env exprs
       return (ExplicitPArr x new_ty new_exprs)
...
zonkExpr env (RecordUpd { rupd_expr = expr, rupd_flds = rbinds
  ...
  = do  { new_expr    <- zonkLExpr env expr
        ...
        ; return (RecordUpd { rupd_ext = noExt
                            , rupd_expr = new_expr, rupd_flds =  new_rbinds
...
zonkExpr env (ArithSeq x expr wit info)
  = do (env1, new_wit) <- zonkWit env wit
       new_expr <- zonkExpr env expr
       new_info <- zonkArithSeq env1 info
       return (ArithSeq x new_expr new_wit new_info)
   where zonkWit env Nothing    = return (env, Nothing)
         zonkWit env (Just fln) = second Just <$> zonkSyntaxExpr env fln

zonkExpr env (PArrSeq x expr info)
  = do new_expr <- zonkExpr env expr
       new_info <- zonkArithSeq env info
       return (PArrSeq x new_expr new_info)
...
zonkExpr env (HsStatic x fvs expr)
  = HsStatic x fvs <$> zonkLExpr env expr
