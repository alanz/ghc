...
processAllTypeCheckedModule tcm = do
  ...
  where
    ...
    getTypeLHsBind :: LHsBind GHCT -> m (Maybe (Maybe (IdP GHCT),SrcSpan,Type))
    ...
    getTypeLHsExpr :: LHsExpr GHCT -> m (Maybe (Maybe (IdP GHCT),SrcSpan,Type))
    getTypeLHsExpr e = do
        hs_env  <- getSession
    ...
    getTypeLPat :: LPat GHCT -> m (Maybe (Maybe (IdP GHCT),SrcSpan,Type))
